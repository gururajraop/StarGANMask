...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader, config):
        ...
        # Loss for mask
        self.criterionMask = networks.MaskLoss('angerr').to(self.device)
    ...
    def train(self):
        ...
        x_fixed, x_target, c_org = next(data_iter)
        ...
        for i in range(start_iters, self.num_iters):
            ...
            try:
                ...
                x_real, x_target, label_org = next(data_iter)
            except:
                data_iter = iter(data_loader)
                x_real, x_target, label_org = next(data_iter)
                # x_real, label_org = next(data_iter)
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake, mask = self.G(x_real, c_trg)
                out_src, out_cls = self.D(x_fake)
                g_loss_fake = - torch.mean(out_src)
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset)
                g_loss_mask = self.criterionMask(x_real, x_target, x_fake, mask)
                ...
                # Backward and optimize.
                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_cls * g_loss_cls + g_loss_mask
                self.reset_grad()
                g_loss.backward()
                ...
                loss['G/loss_mask'] = g_loss_mask.item()
    ...
    def train_multi(self):
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                if (i+1) % self.n_critic == 0:
                    # Original-to-target domain.
                    x_fake, x_mask = self.G(x_real, c_trg)
                    out_src, out_cls = self.D(x_fake)
                    out_cls = out_cls[:, :self.c_dim] if dataset == 'CelebA' else out_cls[:, self.c_dim:]
                    g_loss_fake = - torch.mean(out_src)
